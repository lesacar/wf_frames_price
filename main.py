...
# Write list_of_cheapest_offers to tuple.txt
list_of_cheapest_offers.sort(key=lambda x: x[1], reverse=True)
with open("tuple.txt", 'w') as tupleF:
...
print("CHECK 'tuple.txt' FOR THE SORTED PRIME FRAME SET PRICES")
print("Keep in mind that the 'tuple.txt' file only contains the cheapest price\nwhich could be a troll, be sure to double check prices on wfm")
